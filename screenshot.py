...
def formateWKT(wktstr,fillcolor):
    try:
        ...
        for index in range(len(points)):
            point=points[index].split(' ')
            p=[]
            coordinates=millerToXY(float(point[0]), float(point[1]))
            p.append(coordinates[0])
            p.append(coordinates[1])
            path.append(p)
        patches.append(Polygon(path, True))
        patchcollection=PatchCollection(patches, facecolor= fillcolor, edgecolor=fillcolor, linewidths=1., zorder=2,alpha=0.5)
...
def screenShot(filepath,id,wkt):
    ...
    #加载楼宇
    rows=getTableData("select b.wkt,case when replace(replace(p.p053,char(13),''),char(10),'') = '优势居民区' then 1 when replace(replace(p.p053,char(13),''),char(10),'') = '劣势居民区' then 3 when replace(replace(p.p053,char(13),''),char(10),'') = '持平居民区' then 2 else 0 end p053 from cell_building c left join T_GIS_BUILDING b on c.bid = b.OBJECTID left join view_province_building p on p.building_id = b.[建筑物] where c.aid='"+id+"'")
    for row in rows:
        _patches=createPatches(map,row[0])
        _facecolor='#069F08'
        if row[1]==1:
            _facecolor='#069F08'
        elif row[1]==2:
            ...
            _facecolor='#B21702'
        _patchcollection=PatchCollection(_patches, facecolor=_facecolor, edgecolor='#FFFFFF', linewidths=0.001, zorder=2,alpha=1)
        ax.add_collection(_patchcollection)
...
if __name__ == '__main__':
    ...
    if(dtime.month<10):
        month='0'+month
    zipDir(base_path+'\\'+year+'\\'+month,base_path+'\\'+year+'\\'+month+'.zip','')
