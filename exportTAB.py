...
def createSQL(table,city,county,branch):
    ...
    if table=='10':
        if city=='' or city==None:
            ...
            city="city='"+city+"'"
        sql='select city,name,area,Shape.STAsText() as wkt from T_GIS_MAIN_URBAN_MANAGE where '+city
    if table=='11':
        if city=='' or city==None:
            city='1=1'
        else:
            city="city='"+city+"'"
        sql='select city,name,area,Shape.STAsText() as wkt from T_GIS_GENERAL_URBAN_MANAGE where '+city
    if table=='12':
        if city=='' or city==None:
            city='1=1'
        else:
            city="city='"+city+"'"
        sql='select city,name,area,Shape.STAsText() as wkt from T_GIS_COUNTY_TOWN_MANAGE where '+city
    if table=='13':
        if city=='' or city==None:
            city='1=1'
        else:
...
def getFields(table):
    ...
    if table=='6':
        fields=fields06
    if table=='10' or table=='11' or table=='12' or table=='13':
        fields=fieldsarea
    return fields
...
def drawMap(id,table,foldername,filename,city,county,branch,phone):
    ...
    for i in range(top_left_row_col[0],bottom_right_row_col[0]+1):
        for j in range(top_left_row_col[1],bottom_right_row_col[1]+1):
            try:
                #http://10.110.39.239:8219/tile?x=200&y=106&z=8
...
def drawBaiduMap(id,table,foldername,filename,city,county,branch,phone):
    ...
    _map = Basemap(llcrnrlon=_llcrnrlon,llcrnrlat=_llcrnrlat,urcrnrlon=_urcrnrlon,urcrnrlat=_urcrnrlat,projection = 'merc', epsg=3857)
    _extent=[minxy[0],minxy[1],maxxy[0],maxxy[1]]
    zoom=calcLevel(_extent,_width,_height)-1
    top_left_row_col=xyzToRowCol(_extent[0],_extent[3],zoom)
    bottom_right_row_col=xyzToRowCol(_extent[2],_extent[1],zoom)
    resolution = (earth_radius * 2 / math.pow(2, zoom)) / 256
    min_point = rowcolToXYZ(top_left_row_col[0], bottom_right_row_col[1] + 1, zoom)
...
def writeToTAB(id,table,city,county,branch,phone):
    ...
    if table=='10' or table=='11' or table=='12' or table=='13':
        ...
        if oLayer == None:
            print("图层创建失败！\n")
        _city = ogr.FieldDefn(str(u"地市").encode('gbk'), ogr.OFTString)
        _city.SetWidth(20)
        oLayer.CreateField(_city, 1)
        _name = ogr.FieldDefn(str(u"名称").encode('gbk'), ogr.OFTString)
        _name.SetWidth(50)
